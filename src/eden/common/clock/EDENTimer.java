 ...
 *  {@code (1000 / 60 = 16.67)} which is then truncated to an integral value.
 *  A timer whose delay is set to this value may run either too fast (by 40 ms)
 *  or too slow (by 20 ms), depending on the environment JVM runs on.
 *  <p>
 *  A workaround is to add a housekeeping routine in atop the timer's tasks.
 *  This routine calculates the difference in time between the current and last
 *  tick, then adjusts the timer's delay to either floor, if the difference is
 *  too large, or ceil, if the difference is too small.
 *  <p>
 *  Subclasses are encouraged to utilize the counter for flexibility. Usually,
 *  it is to be incremented on every tick within the range {@code
 *  [0, fireRate - 1]}:
 *  <p>
...
//~~CONSTRUCTORS~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    public EDENTimer(short fireRate) {
        ...
        this.fireRate  = (fireRate < 1) || (fireRate > 1000) ?
            DEFAULT_FIRE_RATE : fireRate;
